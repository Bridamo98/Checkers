...
#verifica si la ficha quedo en un borde y le da un punto adicional
def border(mov,T):
    j=mov[len(mov)-1][1]
    if(j==7 or j==0):
        return 1
...
def player(E, J):
    ...
    if len(M) == 0:
        return None
    M_sorted = deepcopy(M)
    M_sorted = sorted(M_sorted, key=lambda ele: tail(ele, E)+block(ele,E)+crown(ele,J,E)+feed(ele,E)+border(ele,E), reverse=True)
    if M_sorted[0] <= 0:
